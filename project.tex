...
As the reader might be able to guess based on the stipulation on the sum of the rows, we can consider evolutionary graphs to be a stochastic process. At each time step, an \emph{update rule} is used to determine which vertices, or individuals, are chosen for birth and death. Thus, each entry $w_{ij}$ in $W$ represents the probability for $v_i$ to replace $v_j$. 

The simplest such update rule, Birth-Death with Birth bias (BD-B), is introduced and used in \cite{liebermanEvolutionaryDynamicsGraphs2005}. This process is further outlined in the following table.  
\begin{center}
    \begin{tabularx}{\textwidth}{X X}
        \hline 
        Update Rule & Process \\
        \hline
        \multirow{3}{*}{Birth-death with Birth bias (BD-B)} & (1) Pick vertex $v_i$ with probability proportional to $f_i$ \\
         & (2) Pick vertex $v_j$ from neighbors of $v_i$ with probability $w_{ij}$ \\
         &  (3) $v_i$ replaces $v_j$\\
    \end{tabularx}
\end{center}
